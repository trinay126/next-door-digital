/**
 * Next Door Digital – Express backend server
 *
 * Features:
...
app.use((req, res, next) => {
  if (req.method === 'GET' && (req.path === '/' || req.path.endsWith('.html'))) {
    res.setHeader('Link', [
      '</img/logo.jpg>; rel=preload; as=image; type=image/jpeg',
...
async function sendMail(subject, html) {
  ...
  await transporter.sendMail({
    from:    `"Next Door Digital Bot" <${process.env.EMAIL_USER}>`,
...
app.post('/api/contact', formLimiter, async (req, res) => {
  try {
    ...
    await sendMail(`[Next Door Digital Contact] ${esc(subject)}`, html);
